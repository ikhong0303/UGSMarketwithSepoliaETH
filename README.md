...
### 2026-09-23 WebGL 문제 해결 및 확인 상태

아래는 기존 Editor 실습 기록 이후 진행한 **WebGL 대응 작업**입니다. 앞선 실습 성공 기록이 최신 WebGL 빌드의 모든 기능 검증을 의미하지는 않습니다.

| 항목 | 원인 및 반영 내용 | 확인 상태 |
|---|---|---|
| 빌드 시 `Missing Project ID` / 프로젝트 구성원 경고 | Editor의 Services에서 기존 UGS 프로젝트를 재연결 | 사용자 확인: 경고 사라짐 |
| NFT 지갑 연결 | WebGL에서도 Editor용 Reown을 호출하던 경로를 브라우저 MetaMask로 분기. `personal_sign` 서명 후 `Nft_BindWallet` 및 보유분 동기화 | 사용자 확인: NFT 정상 작동 |
| 신화검 NFT 쿠폰 수령 | Editor 전용 차단 제거. WebGL에서 `redeem(bytes32)` 호출, 0 ETH 및 함수 인자 검증, 쿠폰별 전송 기록으로 중복 전송 방지 | 코드·로컬 테스트 확인. 게시 환경 수령은 추가 확인 필요 |
| 골드·전설검 결제 오류 안내 | `RECEIVER_MUST_BE_EOA`를 스크립트 배포 오류로 안내하던 문구 수정. 상점 수신 지갑 설정을 안내 | 사용자 확인: 전설검 지급. 골드 미지급 건은 거래 해시 확인 필요 |
| 결제 확인 버튼 잠김 | 자동 확인 12회 × 8초 대기 동안 전체 버튼을 잠그던 로직 제거. 송금 후 한 번 확인하고 요청 종료 시 버튼 해제 | 컴파일 확인. 재게시 후 버튼 동작 추가 확인 필요 |
| 인벤토리·거래소 스크롤 | Content의 고정 높이 대신 `VerticalLayoutGroup` + `ContentSizeFitter.PreferredSize` 적용. 상단 기준으로 실제 행 높이·개수·간격에 따라 확장 | 컴파일 확인. 항목이 많은 게시 화면에서 추가 확인 필요 |

**변경 파일:** `SceneWalletPanel.cs`, `MythicNftPanel.cs`, `SimpleMarketWallet.jslib`, `PortfolioMarketDemo.cs`, `CloudCode/tests/wallet.test.js`. 최신 지갑 테스트 **9개 통과**, Unity C# 컴파일 통과(기존 Reown 미사용 멤버 경고 2개). 기존 서버 전체 69개 테스트는 과거 기록이며 이번 WebGL 수정 후 전체 재실행한 결과는 아닙니다.

**반영 방법:** C#과 `.jslib` 수정은 WebGL **재빌드 → 재게시 → 게시 페이지 새로고침**이 필요합니다. 이 변경 자체로 Cloud Code JS 재배포나 Cloud Save 원장 초기화는 필요하지 않습니다. 결제 대기 기록을 보존하기 위해 브라우저 사이트 데이터는 삭제하지 마세요.

#### 학생의 NFT 사용 순서

- 이미 학생 지갑으로 전송된 NFT: 게임 로그인 → **NFT지갑연결** → 해당 지갑으로 서명 → **NFT발행확인**. 쿠폰 수령은 필요 없습니다.
- 아직 교환하지 않은 쿠폰: 전체 `MSW1|11155111|계약주소|수령주소|비밀값` 입력 → **신화검NFT받기** → MetaMask에서 발행 가스비 승인 → 거래 완료 후 **NFT발행확인**.
- 입력창 폭 때문에 끝부분만 보일 수 있으므로, 화면에 짧게 보인다는 이유로 쿠폰이 잘렸다고 판단하지 않습니다.

#### 결제 문제를 구분하는 기준

- **`RECEIVER_MUST_BE_EOA`:** 학생 구매 지갑이 아닌 `simple_market/config.receiverAddress`의 온체인 코드 검사에서 차단됩니다. 현재 서버는 `eth_getCode`가 `0x`인 수신 주소만 허용합니다. 스마트 계정 제한을 삭제하는 것으로 해결하지 않습니다.
- **스마트 계정 토글 재활성화:** 반복 원인은 미확정입니다. 수신 주소 `0xf2548C84990758432d0Dc9BC5F9aC810fEb491E4`에서 코드가 존재하는 상태와 해제 후 `0x` 상태를 각각 관측했습니다. 한 번의 `0x` 조회만으로 재발까지 해결됐다고 판단하지 않습니다. 다시 켜졌을 때 해제 전 온체인 상태 및 활성화·해제 거래를 확인해야 합니다.
- **`RPC_RECEIPT_NOT_FOUND`:** 서버가 해당 결제 거래의 채굴 영수증을 아직 찾지 못한 상태입니다. 지급 완료가 아닙니다. MetaMask 활동에서 거래 상태·해시를 확인하고 해당 상품의 결제 확인을 재시도합니다. 골드와 전설검은 별도 거래이므로 전설검 지급 성공만으로 골드 거래까지 성공했다고 판단하지 않습니다. 재구매·대기 기록 초기화로 해결하지 않습니다.
