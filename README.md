...
v2 실습 기록 갱신일: **2026-09-23**. Economy를 사용할 수 없게 된 수업 환경을 복구하기 위해 Cloud Save + Cloud Code 중심으로 전환했습니다. 이번 환경에서 회원가입·로그인·인벤토리, 거래소 조회·등록·취소·구매·정산, QR 지갑 연결, 골드·전설검 결제, NFT 서명 연결·기존 보유 동기화·새 쿠폰 등록과 수령까지 사용자 실습으로 확인했습니다. **NFT 지갑 간 전송 후 양쪽 게임 인벤토리 반영은 이번에 실행하지 않았습니다.** 학생별 새 환경에서는 아래 순서대로 다시 확인합니다.
...
| 이전 로컬 검증 기록 | 9월 22일 인수인계: 서버·지갑·NFT 69개 통과, Unity 소스 컴파일 및 Push 패키징 성공. 이번 PC에서는 NFT 의존성 설치 전 일반 테스트 59개 통과를 재확인 |
| 현재 환경 사용자 실습 | 가입·로그인·인벤토리, 거래소 등록/취소/다른 계정 구매/정산, QR 연결, 골드/전설검 결제 성공 |
| 현재 환경 NFT 실습 | NFT 서버 3개 배포 및 설정 추가, 서명 연결, 기존 NFT 표시, 새 쿠폰 등록·Unity 수령·인벤토리 반영 성공 |
| 남은 실제 검증 | NFT 지갑 간 전송 후 양쪽 인벤토리 반영, 학생 다수 동시 접속, Push 온라인 동작. 결제 재확인·재로그인 보존도 학생별 체크리스트로 확인 |
...
2026-09-22 수정본은 **Economy 활성화·상품 등록·Publish가 필요 없습니다.** Authentication, Cloud Save, Cloud Code를 사용합니다. Unity는 2026-09-08부터 Economy 신규 프로젝트 등록을 중단했습니다. 공식 안내상 기존 활성 구성 프로젝트는 계속 사용할 수 있으므로 서비스 전체가 삭제됐다는 뜻은 아닙니다. 이 수업은 새 환경에서도 구성할 수 있도록 Economy 의존성을 제거했습니다. [공식 안내](https://docs.unity.com/en-us/economy)
...
이 프로젝트에서 “Economy가 없어졌다”는 것은 기존 수업의 Economy 의존 흐름을 더 이상 사용할 수 없어 제거했다는 의미입니다. 블록체인의 NFT나 ETH가 함께 삭제됐다는 뜻은 아닙니다.

| 기능 | v2 처리 위치와 원리 |
|---|---|
| 회원가입·로그인 | Authentication이 Player ID를 발급하고 Cloud Code 호출자를 확인 |
| 골드·일반 아이템 | Cloud Save Private의 `classroom_market/state.players`에 보관 |
| 거래소 | Cloud Code가 소유자·가격·잔액·매물 상태를 검사하고 아이템 이동과 상태 변경을 함께 저장 |
| 중복 요청 방지 | C#이 `request_id`를 저장·재사용하고 JS가 처리 결과를 `operations`에 저장 |
| 동시 수정 방지 | JS가 Cloud Save `writeLock`으로 저장 충돌을 감지하고 최신 상태에서 재검증 |
| ETH 결제 지급 | Sepolia 거래를 JS로 검증한 뒤 골드/아이템과 영수증을 같은 상태에 저장 |
| NFT | 소유권은 Sepolia 계약에 유지. 서명으로 계정과 지갑을 연결하고 서버가 보유 결과를 게임 인벤토리에 반영 |

Unity의 C#은 UI·인증·지갑 요청·서버 호출을 담당합니다. 서버 원본 JS는 `CloudCode/src/`, NFT 서버는 `CloudCode/nft/`에 있고, Dashboard에는 `deploy`의 완성된 JS를 배포합니다. `NFTWorkshop/web/app.mjs`는 브라우저에서 실행하는 관리자/학생 도구이며 Cloud Code에 올리지 않습니다. `.sol`은 Sepolia에 배포하는 NFT 계약입니다. C# 수정, JS Publish, NFT 계약 배포는 서로 다른 작업입니다.

### 7.2 학생용 진행 순서와 통과 기준

각 단계가 통과한 뒤 다음 단계로 진행합니다. 공용 수업에서는 강사가 서버 설정을 한 번 준비하고 학생은 자신의 게임 계정과 지갑을 사용합니다.

1. **Unity 준비:** 5~6절대로 프로젝트·Scene 1을 열고 UGS Project ID와 production 환경을 맞춥니다.
2. **거래소 저장소:** 8.1절의 Private `state`를 최초 한 번 만듭니다.
3. **거래소 서버:** 9.1절의 `Mkt_*` 8개를 코드·Parameters까지 설정하고 Publish합니다.
4. **게임 계정:** Signup/Login 후 1,000 COIN과 인벤토리를 확인합니다. Random Item으로 일반 아이템을 준비합니다.
5. **목록 조회:** 거래소 Refresh 후 `거래소 로드: N개`를 확인합니다. 새 환경의 0개도 정상입니다.
6. **등록·취소:** 100 COIN으로 판매해 인벤토리에서 빠지고 매물에 나타나는지, Cancel 후 한 개만 돌아오는지 확인합니다.
7. **구매·정산:** 다시 등록하고 다른 게임 계정으로 구매합니다. 구매자 100 COIN 차감·아이템 획득, 판매자 Claim 100 COIN 증가와 중복 Claim 방지를 확인합니다.
8. **결제 준비:** 8.2절 `config`, 결제 JS 4개, 11절 Reown을 설정합니다. 학생 구매 지갑과 상점 수신 지갑을 구분합니다.
9. **QR 연결:** 휴대폰 MetaMask로 연결해 Unity의 지갑 주소와 Sepolia 잔액을 확인합니다.
10. **골드·전설검:** 12절대로 각각 0.0001 Sepolia ETH 결제와 지급을 확인합니다. 지급 지연 시 구매가 아니라 결제 확인 버튼을 누릅니다.
11. **NFT 서버:** 13.5절의 `nft_config`, `nft_state`와 NFT JS 3개를 준비합니다. 결제용 `config`만으로는 NFT 연결이 되지 않습니다.
12. **NFT 서명 연결:** NFT지갑연결 → MetaMask 서명 → 서버 동기화. 이미 보유한 NFT는 새 쿠폰 없이 인벤토리에 반영됩니다.
13. **새 NFT 수령:** 강사가 기존 계약으로 쿠폰을 등록하고 학생이 Unity에서 쿠폰을 사용합니다. 기존 1개를 가진 학생이 1개를 더 받으면 총 2개를 확인합니다.
14. **지갑 표시:** MetaMask 자동 목록에 없으면 13.10절의 NFT 가져오기를 사용합니다.
15. **전송 검증:** 13.8절대로 다른 지갑에 보내고 각각 별도 게임 계정에서 동기화합니다. 이번 복구 세션에서는 이 단계가 미실행 상태입니다.
...
**이번 실습에서 취소 실패의 원인은 Dashboard Parameters의 `request_id` 누락이었습니다.** JS 본문을 붙여넣는 것만으로 끝내지 말고 아래 표의 이름·타입·Required를 확인한 뒤 Publish하세요. 구매·취소·정산 등 쓰기 함수마다 별도로 설정합니다.
...
### 12.0 수신 지갑과 스마트 계정 확인

이번 실습에서 `RECEIVER_MUST_BE_EOA`는 결제 함수 미배포가 아니라 **수신 지갑에 EIP-7702 스마트 계정 코드가 존재해서** 발생했습니다. Google 로그인 자체의 오류가 아닙니다. 같은 지갑을 대상으로 RPC `eth_call`을 실행했을 때 일반 ETH 송금은 성공했지만, 프로젝트의 계정 식별 메모를 붙인 송금은 `execution reverted`로 거부됐습니다. 실제 송금 없이 확인한 결과이며 서버 제한만 삭제하는 것으로 해결되지 않았습니다.

현재 결제는 송금 data에 상품·UGS 프로젝트·환경·Player ID를 넣어 다른 계정의 거래를 재사용하지 못하게 합니다. 따라서 ETH 전송이 가능한 모든 스마트 지갑이 이 결제 방식과 호환되는 것은 아닙니다. 현재 코드는 수신 주소의 `eth_getCode` 결과가 `0x`일 때만 허용합니다. 이 제한은 보관된 Economy 구버전 코드에도 있습니다.

해당 오류가 발생한 수신용 MetaMask에서는 다음과 같이 처리했습니다.

1. **상점 수신 지갑을 관리하는 MetaMask**를 엽니다. Unity에 연결된 학생 지갑과 구분합니다.
2. 상단 계정 선택 → 해당 계정 옆 **⋯ → 계정 세부정보 → Smart account**를 엽니다. 전체 설정 화면과 다릅니다. 버전에 따라 계정 메뉴에 Smart account가 바로 표시됩니다.
3. **Sepolia의 스마트 계정 기능을 끄고**, 표시되는 변경 거래를 승인합니다. Sepolia 가스비가 필요합니다.
4. 거래 완료 후 같은 수신 주소의 코드가 `0x`가 되었는지 확인합니다. 주소나 Cloud Save 설정을 바꾸거나 새 지갑을 만들 필요는 없습니다.
5. Unity에서 구매를 다시 시도합니다. 이번 환경에서는 이 조치 후 골드와 전설검 결제가 모두 성공했습니다.

[MetaMask 공식 스마트 계정 해제 안내](https://support.metamask.io/configure/accounts/switch-to-or-revert-from-a-smart-account/). 모든 오류에서 이 작업을 할 필요는 없으며 `RECEIVER_MUST_BE_EOA`와 수신 주소 상태를 먼저 확인합니다. `rpcUrl`은 `https://ethereum-sepolia-rpc.publicnode.com`이라는 URL 문자열 그대로 저장하며 마크다운 링크 문법을 넣지 않습니다.
...
3. 현재 ZIP에는 실행용 `web/index.html`, `app.mjs`, `ethers.min.js`, `contract.json`이 들어 있습니다. 제공된 도구를 그대로 실행할 때는 의존성 설치나 재빌드 없이 다음을 실행합니다.
...
**기존 수업 계약을 사용할 때는 아래 새 배포 단계를 건너뜁니다.** 이번 복구에서 사용한 값은 다음과 같습니다. 다른 수업에서는 강사가 지정한 값을 사용하세요.

| 역할 | 이번 환경의 주소 | 어디에 사용하는가 |
|---|---|---|
| NFT 계약 | `0xd80054858B4116A15977B9DA5e27C507d1381A6e` | 웹 맨 위 계약 주소, Unity Bridge의 Mythic Nft Contract, Cloud Save `nft_config.contractAddress` |
| 계약 관리자 | `0xf2548C84990758432d0Dc9BC5F9aC810fEb491E4` | 쿠폰 발급 시 PC MetaMask에서 선택할 계정. 별도 입력칸 이름이 아님 |
| 학생 수령 지갑 예시 | `0x8395aaBAc6864d7046CD56c6C42EbDbc77ADD1E5` | 받을 학생의 공개 지갑 주소. 학생마다 자신의 Unity 연결 주소를 사용 |

계약 주소는 Scene 1에 저장돼 있던 값이고, 관리자 주소는 계약의 `owner()` 조회로 확인했습니다. 이번 환경에서는 관리자가 상점 결제 수신 지갑도 겸하지만 두 역할이 반드시 같아야 하는 것은 아닙니다. **관리자 지갑 주소를 계약 주소 칸에 넣으면 `이 주소에 계약이 없습니다`가 발생합니다.**

기존 계약을 쓸 때: PC MetaMask에서 관리자 계정을 선택 → 웹의 MetaMask 연결 → 웹에 표시된 연결 주소 확인 → 맨 위 계약 주소 입력 → 13.3절로 이동합니다. 학생 지갑으로 NFT 보유를 조회한 뒤 쿠폰을 발급하려면 관리자 계정으로 전환하고 다시 연결합니다.

새 수업용 계약이 필요한 경우에만 다음을 진행합니다.
...
**쿠폰이 화면에 나타났다고 등록이 끝난 것은 아닙니다.** 도구는 거래 전송 전에 쿠폰 초안을 표시합니다. MetaMask의 `보류 중`과 웹의 블록 확인이 끝나고 `쿠폰 등록 완료`가 표시되는지 확인하세요. 지연 시 반복 발급하지 말고 마지막 txHash와 등록 상태를 조회합니다.

5개를 만들려면 현재 도구에서는 등록을 한 장씩 총 5번 수행합니다. **각 등록 완료 후 쿠폰을 별도로 복사·보관한 다음** 다음 장을 만드세요. 화면과 “마지막 쿠폰 불러오기”에는 마지막 한 장만 남습니다. 같은 학생에게 5개를 주려면 같은 수령 주소를 사용하고, 여러 학생이면 매번 수령 주소를 바꿉니다. 쿠폰 1장 사용 시 NFT 1개가 발행됩니다.
...
### 13.10 기존 NFT 확인과 MetaMask 표시

Economy에 있던 아이템 기록과 블록체인의 NFT 소유권은 별개입니다. 이번 실습에서는 이전 버전에서 발행받은 NFT가 지갑에 남아 있었고, v2의 NFT지갑연결과 동기화만으로 인벤토리에 나타났습니다. 같은 NFT를 복구한다는 이유로 새 쿠폰을 발급하지 않습니다.

1. PC NFT 도구에서 **NFT를 가진 학생 계정**으로 MetaMask를 연결합니다. 관리자가 대신 조회하는 화면이 아니라 현재 연결 계정의 목록입니다.
2. 맨 위에 기존 NFT 계약 주소를 넣습니다.
3. **4. 보유 확인 및 전송 → 내 NFT 목록 조회**를 누릅니다.
4. 보유 수와 tokenId를 확인합니다. 이번 기존 NFT의 결과는 **보유 1개, tokenId 1**이었습니다.
5. MetaMask의 해당 계정·Sepolia에서 **NFT → NFT 가져오기**를 열고 같은 계약 주소와 tokenId를 입력합니다.
6. 이름과 이미지 표시를 확인합니다. 가져오기는 지갑 화면에 자산을 등록하는 것이며 NFT를 새로 발행하거나 전송하지 않습니다. 이미지 표시 여부와 실제 소유권은 별개입니다.

NFT 도구는 사이트 연결 체인이 Sepolia가 아니면 전환 요청을 띄웁니다. MetaMask의 자산 목록에 Sepolia가 보인다는 사실만으로 웹 연결이 완료된 것은 아닙니다. 수정한 웹 코드를 적용하려면 페이지 새로고침 후 **MetaMask 연결**을 다시 누르고 연결 주소 표시를 확인합니다. **받는 사람 공개 지갑 주소**는 전송용 칸이므로 MetaMask 가져오기를 위해 계약 주소를 이곳에 입력하지 않습니다.

## 14. 장애 확인 및 복구 원칙

### 14.0 이번 복구에서 확인한 오류와 수정

| 증상 | 확인된 원인·조치 |
|---|---|
| 거래소 `Could not find member 'revision'` | JS 응답은 `{revision, listings}`인데 C# DTO에 revision이 없었음. `PortfolioMarketDemo.MarketListResult`에 `long revision` 추가. 최신 C# 사용, 이 수정만으로 JS 재배포는 불필요 |
| 본인 매물 Cancel에서 `INVALID_REQUEST_ID` | Dashboard의 해당 함수 Parameters를 보완하고 Publish한 뒤 성공. `listing_id`, `request_id` 모두 String 필수 |
| QR 승인 직후 “지갑 연결 창을 닫았습니다” | QR 창 닫힘과 계정 연결 상태 갱신의 시간차 가능성. `ReownWalletBridge`에서 연결 상태 재확인 및 창 닫힘 후 최대 3초 유예 추가. 연결 성공은 확인했으나 이 시간차의 반복 재현 검증은 별도 |
| QR 스캔 후 휴대폰 로딩만 표시 | 승인 화면 도달 여부·Unity Play/Pause·Console 확인. 새 QR, 앱 재시작, 휴대폰 네트워크 변경은 연결 진단 방법이며 특정 네트워크가 원인이었다고 확정하지 않음 |
| 수신 지갑과 다른 구매자 지갑을 사용하세요 | 실제 구매 주소와 수신 주소가 같았음. 역할 분리. 같은 메시지는 잘못된 수신 주소 형식에도 발생하므로 실제 값 확인 |
| `RECEIVER_MUST_BE_EOA` | 수신 지갑의 스마트 계정 코드를 확인. 12.0절대로 Sepolia 스마트 기능 해제 후 골드·전설검 결제 성공 |
| NFT 연결 `404 Script could not be found` | NFT JS 3개가 없었음. 13.5절의 이름·Parameters로 생성하고 Publish |
| 이후 `NFT_CONFIG_REQUIRED` | `simple_market/Default`에 NFT 설정이 없었음. `nft_config`와 신규 `nft_state` 추가 후 서명 연결 성공 |
| MetaMask에는 Sepolia 표시, 웹은 네트워크 오류 | 표시 필터와 사이트 연결 체인은 별도로 확인. `web/app.mjs`에서 Sepolia 전환 요청 후 체인 재검사하도록 수정. 웹 새로고침·재연결 후 조회 성공 |
| 쿠폰 발급 전 “이 주소에 계약이 없습니다” | 웹 맨 위에는 관리자 주소가 아니라 NFT 계약 주소를 입력. 관리자 계정은 MetaMask에서 선택 |
| MetaMask에서 NFT가 안 보임 | 체인에는 기존 NFT 1개/tokenId 1 보유 확인. NFT 가져오기로 표시. 새 발행 필요 없음 |

아래 오류표와 함께 사용합니다. UI의 “Cloud Code 배포 확인” 같은 공통 문구만으로 원인을 단정하지 말고 Console의 실제 오류 코드를 확인합니다. 예를 들어 422는 함수가 실행됐으나 검증에서 실패한 경우이고, 404는 호출한 스크립트를 해당 프로젝트·환경에서 찾지 못한 경우입니다.
...
이번 PC의 Node 22.18.0에서는 `--test-isolation=none`이 지원되지 않았습니다. 위 직접 실행 명령은 해당 옵션을 제외했습니다. 기존 package.json의 `npm test` / `test:sync`에는 그 옵션이 남아 있으므로 같은 오류가 나면 옵션을 지원하는 Node를 사용하거나, NFTWorkshop에서 `node --test tests/contract.test.mjs`, `node --test ../CloudCode/nft/tests.cjs`로 실행합니다. NFT 테스트에는 먼저 `npm.cmd ci`가 필요하며, 단순 웹 도구 실행에는 필요하지 않습니다.

이번 PC에서는 거래소·결제·지갑 관련 59개 테스트가 통과했고, 초기 NFT 테스트 실행은 ethers 의존성 부재로 완료하지 못했습니다. 위 69개 및 Unity 컴파일 기록은 이전 환경의 기록입니다. 이후 실제 사용자 실습에서 거래소·결제·새 NFT 수령까지 확인한 범위는 1.3절과 같습니다. NFT 지갑 간 전송, 모든 신규 학생 PC, Push 온라인 동작까지 검증됐다는 뜻은 아닙니다. 이 README 갱신 자체는 서버 배포를 수행하지 않습니다.
...
### 강사용 메모: 2026-09-23 복구 실습 완료 범위

- **사용자 실습 성공:** 가입·로그인·개인 인벤토리, 거래소 조회·등록·취소·다른 계정 구매·판매대금 정산, Reown QR 연결, 10,000골드·전설검 결제, NFT 지갑 서명 연결, 기존 신화검 동기화, 웹 보유 조회, MetaMask NFT 가져오기, 새 쿠폰 등록·Unity 수령·인벤토리 반영.
- **기존 자산 유지:** NFT 계약은 새로 배포하지 않았습니다. 기존 지갑 보유 NFT가 v2에 표시됐고, 새 쿠폰으로 추가 NFT 수령도 확인했습니다.
- **로컬 코드 수정:** 거래소 응답 DTO에 revision 추가, QR 창 닫힘 판정 보완, NFT 웹의 Sepolia 전환 요청 추가. 파라미터 누락은 Dashboard에서 수정했고 NFT JS 3개 및 설정 키 2개를 추가했습니다.
- **미실행:** 다른 지갑 전송 후 A 인벤토리 제거/B 인벤토리 추가는 예상만으로 완료 처리하지 않습니다. 13.8절의 학생 검증 항목으로 남깁니다. 중복 결제 지급 방지·재로그인 보존의 실제 개별 확인 결과도 별도로 기록합니다.
- **수업 재현:** 현재 환경에서 성공한 사실과 다른 학생의 새 프로젝트 설정 완료는 다릅니다. 공용 프로젝트·환경을 맞추고 7.2절을 순서대로 수행하세요.

### 강사용 메모: 보유 NFT가 MetaMask에 표시되지 않을 때

NFT를 실제로 보유하고 있어도 MetaMask의 NFT 목록에는 자동으로 표시되지 않을 수 있습니다. 이번 실습에서는 Economy 사용 당시 발행받은 신화검 NFT가 Sepolia 지갑에 그대로 남아 있었고, MetaMask에서 수동으로 가져온 뒤 이름과 검 이미지가 표시됐습니다. 새로 발행하거나 NFT 데이터를 수정한 것이 아닙니다.

**MetaMask 표시 방법**

1. NFT를 보유한 계정을 선택하고 Sepolia 네트워크를 확인합니다.
2. **MetaMask → NFT → NFT 가져오기**를 엽니다.
3. 아래 계약 주소와 토큰 ID를 입력합니다.

| 항목 | 이번 실습에서 확인한 값 |
|---|---|
| 계약 주소 | `0xd80054858B4116A15977B9DA5e27C507d1381A6e` |
| 토큰 ID | `1` |

위 값은 이번에 확인한 NFT의 예시입니다. 다른 NFT는 해당 계약 주소와 실제 tokenId를 사용합니다. tokenId를 모르면 NFT 실습 웹페이지에 보유 지갑을 연결하고 계약 주소를 입력한 뒤 **4. 보유 확인 및 전송 → 내 NFT 목록 조회**에서 확인합니다. MetaMask에 표시하기 위해 NFT를 전송하거나 쿠폰을 새로 만들 필요는 없습니다. 웹페이지의 **받는 사람 공개 지갑 주소**는 전송용 입력란이며, NFT 가져오기용 계약 주소 입력란이 아닙니다.

**Unity에서는 먼저 보였던 이유**

Unity는 MetaMask의 표시 목록을 읽지 않습니다. NFT 지갑 연결 후 서버가 Sepolia의 실제 보유 여부를 조회하고 Cloud Save 인벤토리에 반영합니다. 따라서 MetaMask 목록이 비어 있어도 Unity 인벤토리에는 신화검이 나타날 수 있습니다. Economy 데이터가 사라져도 블록체인에 기록된 NFT 소유권은 별도로 유지됩니다.

**웹페이지 연결 수정과의 구분**

이번에 수정한 것은 NFT 실습 웹페이지의 Sepolia 연결 처리입니다. 사이트에 연결된 네트워크가 Sepolia가 아니면 전환 요청을 띄우도록 변경했습니다. 이는 웹페이지의 조회 연결을 위한 수정이며, MetaMask에서 NFT를 수동으로 가져와 표시하는 과정과는 별개입니다.
